...
class FileFailingHostsCache implements \WC_Order_Search_Admin\AlgoliaSearch\FailingHostsCache
{
    /**
    ...
    private function loadFailingHostsCacheFromDisk()
    {
        ...
        // Some basic checks.
        if (!isset($data[self::TIMESTAMP]) || !isset($data[self::FAILING_HOSTS]) || !is_int($data[self::TIMESTAMP]) || !is_array($data[self::FAILING_HOSTS])) {
            return array();
        }
        ...
        $elapsed = time() - $data[self::TIMESTAMP];
